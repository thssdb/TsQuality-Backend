...
public class HdfsStorageEngineTest {
  ...
  private void thenResultShouldHaveBeenPersisted(TsFileInfo tsFileInfo, Map<Path, TsFileStat> stats) throws Exception {
  ...
  private void thenSeriesShouldHaveBeenPersisted(Map<Path, TsFileStat> stats) throws Exception {
    ...
    Dataset<Row> dataset = spark.read().option("header", true).csv(HdfsStorageConstants.seriesFilePath);
    ...
    assertThat(paths).containsExactlyElementsOf(SeriesStatGenerator.PATHS.stream().map(Path::getFullPath).toList());
  }
  ...
  private void thenFileSeriesStatsShouldHaveBeenPersisted(TsFileInfo tsFileInfo, Map<Path, TsFileStat> stats) throws Exception {
    assertThat(HdfsUtils.checkFileExists(HdfsStorageConstants.fileSeriesStatsFilePath, conf)).isTrue();
  }

  private void thenChunkSeriesStatsShouldHaveBeenPersisted(TsFileInfo tsFileInfo, Map<Path, TsFileStat> stats) throws Exception {
    assertThat(HdfsUtils.checkFileExists(HdfsStorageConstants.chunkSeriesStatsFilePath, conf)).isTrue();
  }
}
